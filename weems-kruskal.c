...
/* 
	His data enters with 11 vertices and 23 edges but the edges are the total original edges
    and not the number in the resulting mst which will be one less than the vertices. 
*/
...
int find(int x){	// Find root of tree containing x
...
void unionFunc(int i,int j){// i and j must be roots!
...
int weightAscending(const void* xin, const void* yin){	// Used in call to qsort()
	edgeType *x, *y;
...
int main(){
	...
	for (i = 0; i < numEdges; i++) scanf("%d %d %d", &edgeTab[i].tail, &edgeTab[i].head, &edgeTab[i].weight);
	...
	if(numTrees != 1) printf("MST does not exist\n");
